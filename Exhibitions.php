<?php include $_SERVER['DOCUMENT_ROOT'] . "/clicon" . "/Processes/Functions.php" ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>Clicon_Exhibitions</title>
    <?php include $_SERVER['DOCUMENT_ROOT'] . "/clicon" . "/Shared/Head.php"; ?>
    <link rel="stylesheet" href="/clicon/Style/ExhibitionStyle.css">
</head>
<body>
<?php include $_SERVER['DOCUMENT_ROOT'] . "/clicon" . "/Shared/Header.php"; ?>
<main>
    <!--    RETRO / AAA-->
    <div class="row">
        <div class="col-lg-1"></div>
        <div class="col-lg-5" style="padding: 0">
            <div class="boxIndex" id="retro">
                <div style="position: relative; height: 100%">
                    <p>
                        For those warriors whose way of the samurai can only be expressed through a keyboard and a
                        mouse,
                        drop by the LAN Station!
                        <br>
                        Come by for some fun & enter our RTS or FPS Tournaments where your skills will be put to the
                        test to
                        see who will be the best of the best.
                    </p>
                    <h3>
                        <b><?php echo($lang("exhib_retro")); ?></b>
                    </h3>
                </div>
            </div>
        </div>
        <div class="col-lg-5" style="padding: 0">
            <div class="boxIndex" id="aaa">
                <div style="position: relative; height: 100%">
                    <p>
                        For those warriors whose way of the samurai can only be expressed through a keyboard and a
                        mouse,
                        drop by the LAN Station!
                        <br>
                        Come by for some fun & enter our RTS or FPS Tournaments where your skills will be put to the
                        test to
                        see who will be the best of the best.
                    </p>
                    <h3>
                        <b><?php echo($lang("exhib_studios")); ?></b>
                    </h3>
                </div>
            </div>
        </div>
        <div class="col-lg-1"></div>
    </div>
    <!--    LAN PARTY-->
    <div class="row">
        <div class="col-lg-1"></div>
        <div class="col-lg-10" style="padding: 0">
            <div class="boxIndex" id="lan" style="background-position: center center">
                <div style="position: relative; height: 100%">
                    <p>
                        For those warriors whose way of the samurai can only be expressed through a keyboard and a
                        mouse,
                        drop by the LAN Station!
                        <br>
                        Come by for some fun & enter our RTS or FPS Tournaments where your skills will be put to the
                        test to
                        see who will be the best of the best.
                    </p>
                    <h3>
                        <b><?php echo($lang("exhib_lan")); ?></b>
                    </h3>
                </div>
            </div>
        </div>
        <div class="col-lg-1"></div>
    </div>
    <!--    ARCADE / INDIE-->
    <div class="row">
        <div class="col-lg-1"></div>
        <div class="col-lg-5" style="padding: 0">
            <div class="boxIndex" id="arcade">
                <div style="position: relative; height: 100%">
                    <p>
                        For those warriors whose way of the samurai can only be expressed through a keyboard and a
                        mouse,
                        drop by the LAN Station!
                        <br>
                        Come by for some fun & enter our RTS or FPS Tournaments where your skills will be put to the
                        test to
                        see who will be the best of the best.
                    </p>
                    <h3>
                        <b><?php echo($lang("exhib_arcade")); ?></b>
                    </h3>
                </div>
            </div>
        </div>
        <div class="col-lg-5" style="padding: 0">
            <div class="boxIndex" id="indie">
                <div style="position: relative; height: 100%">
                    <p>
                        For those warriors whose way of the samurai can only be expressed through a keyboard and a
                        mouse,
                        drop by the LAN Station!
                        <br>
                        Come by for some fun & enter our RTS or FPS Tournaments where your skills will be put to the
                        test to
                        see who will be the best of the best.
                    </p>

                    <h3>
                        <b><?php echo($lang("exhib_indie")); ?></b>
                    </h3>
                </div>
            </div>
        </div>
        <div class="col-lg-1"></div>
    </div>
</main>
<?php include $_SERVER['DOCUMENT_ROOT'] . "/clicon" . "/Shared/Footer.html"; ?>
</body>
</html>